...
type APIResult = {
  ...
  data?: BasicObject | ReadableStream<Uint8Array> | string | ArrayBuffer | null;
...
async function handleResponse(
  response: Response
): Promise<BasicObject | string | ArrayBuffer | ReadableStream<Uint8Array>> {
  ...
  if (contentType && contentType.includes('application/json')) {
    ...
    return response.body as ReadableStream<Uint8Array>; // Return as a stream
  } else {
    return response.arrayBuffer(); // Fallback to ArrayBuffer
  }
}
...
export async function get(
  ...
  const result: APIResult = { data: null, status: 0, headers: {} };
...
export async function post(
  ...
  url = await handleURL(url);
  const result: APIResult = { data: null, status: 0 };
