...
async function checkForUpdates(productId, resourcesPath) {
    const repoOwner = 'Code-Czar';
    const repoName = 'clients-auto';
    const destinationPath = path_.normalize(path_.resolve(resourcesPath, '.quasar', 'resources'));
    const localLatestFilePath = path_.normalize(path_.resolve(destinationPath, 'latest.yml'));
    const tempFolderPath = path_.normalize(path_.resolve(destinationPath, 'tempFolder'));
    ...
    try {
        ...
        const releaseResponse = await axios_.get(`https://api.github.com/repos/${repoOwner}/${repoName}/releases/latest`, {
            headers: {
        ...
        const latestAsset = latestRelease.assets.find((asset) => asset.name === 'latest.yml');
        if (!latestAsset) {
        ...
        const latestYmlResponse = await axios_.get(`https://api.github.com/repos/${repoOwner}/${repoName}/releases/assets/${latestAsset.id}`, {
            headers: {
                Authorization: `Bearer ${githubToken}`,
        ...
        if (!localVersion || latestVersion !== localVersion) {
            ...
            console.log('🚀 ~ checkForUpdates ~ localLatestFilePath:', localLatestFilePath);
            console.log('🚀 ~ checkForUpdates ~ latestYmlContent:', latestYmlContent);
            fs_.writeFileSync(path_.join(localLatestFilePath), latestYmlContent, 'utf8');
...
async function cloneRepository(productId, resourcesPath) {
    ...
    const downloadUrl = 'https://api.github.com/repos/Code-Czar/clients-auto/zipball/main';
    // Get GitHub Token
    const githubToken = await getGithubToken(productId);
    if (!githubToken) {
        console.error('Failed to retrieve a valid GitHub token.');
...
async function buildAndRunDocker(resourcesPath) {
    ...
    const dockerComposeFile = path_.normalize(path_.join(destinationPath, 'tempFolder', 'docker-compose.yml'));
    console.log('🚀 ~ buildAndRunDocker ~ dockerComposeFile:', dockerComposeFile);
    try {
        fs_.accessSync(dockerComposeFile, fs_.constants.F_OK);
    }
    catch (error) {
        console.error(`BUILDER : Docker Compose file not found at ${dockerComposeFile}`);
        return;
    }
    try {
